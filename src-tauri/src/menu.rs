// ネイティブのメニューバーと「について」（参照実装 ui/menus.py の役目）。
// 19-2 で lib.rs から分けた。lib.rs には Tauri の組み立て（run）だけ残す
...
pub(crate) fn build_menu(app: &tauri::App) -> tauri::Result<()> {
    use tauri::menu::{
...
mod tests {
    ...
    fn test_印つきメニューの_id_は_TS_と同じ見本に揃っている() {
        ...
        let flat: String = include_str!("menu.rs")
            .chars()
