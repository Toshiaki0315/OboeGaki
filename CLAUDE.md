...
実装タスクの一覧と進捗は [docs/TASKS.md](docs/TASKS.md)、参照実装との
機能差分の全量は [docs/hitofude-gap.md](docs/hitofude-gap.md)。
...
## 6.5 環境設定「PowerPoint」タブ（TASKS 第 8 群）

仕様は [docs/OboegakiPptxSettingsSpec.md](docs/OboegakiPptxSettingsSpec.md)、
**覚書への載せ方は [ADR-0046](docs/adr/0046-pptx-settings-tab.md)**。
項目 ID（CFG-xx / UI-xx / GR-xx / ST-xx / VA-xx）を実装とテストの名前に書く。

- 設定に置くのは「**文書をまたいで一貫させたいもの**」だけ。文書ごとに
  変わるもの（どこで分けるか・どのレイアウトか）は Markdown の構造から決める
- **効かないつまみを画面に出さない。** 設定は逃げ道であって、既定値の
  代わりではない。既定のまま書き出して綺麗、を保つ
- 用紙で変わる数値は必ず `slideMetrics()` / `deriveGrid()` から取る。
  `pptx.ts` に 13.333 や 0.7 を書かない（GR-01）
- 色は `{ ref: "accent1" } | { hex: "1E2761" }`。**既定はテーマ参照**で、
  触った色だけ具体色になる。具体色は `#` なし 6 桁の大文字に正規化する
  （CFG-17。pptxgenjs は `#` 付き・8 桁で壊れたファイルを作る）
- 文書ごとの上書きは **front matter**（`slide-font` など）。**アプリから
  書き込まない**（SC-04 の狙いはこれで満たす）。サイドカーは作らない —
  `.md` の外に「捨てられないもの」を増やさない（T1 / T7）
- 優先順位は front matter > 環境設定（SC-02 > SC-01）
- 設定を足したら、**出来上がった `.pptx` の XML** で確かめるテストも足す
  （画面ではなく結果で見る）
