...
- [x] **20-3. 性能の再計測**（2026-09-18。docs/bench.md に記録）
      ...
      - 打鍵: 内蔵ブラウザ（Chromium、**非表示タブ**）で p50 3.4ms / p95 16〜23ms。
            基準割れに見えるが、リファクタ前（fcd11b1）を同じ環境・同じ順で測ると
            同じ値（p50 3.4 / p95 16）。**差はない**が、この環境では 16ms の判定が
            できない（非表示タブは CPU を絞られる。最初の 1 回だけ速いのはその証拠）。
