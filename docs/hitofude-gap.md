...
| `Cmd` で押せると見せる（指差しのカーソル）                                               | editor_widget.\_update_hover                               | 実装済み（activation.ts の activationCursor。実機報告 2026-09-13 で気付いた抜け）                                 |
| `Cmd` で触れたときの覗き見（冒頭が泡で出る）                                             | editor_widget.\_link_preview                               | 実装済み（note-peek.ts。形と同じ合図から動かす — 触れてから `Cmd` を押しても出る）                                |
...
| 機能                                                                             | 根拠                               | 状態                                                                                    |
| -------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------- |
| 環境設定ダイアログ（フォントサイズ・履歴の間隔・ゴミ箱の日数・本文幅・テーマ等） | ui/preferences.py, N-1             | 実装済み（components/PreferencesDialog.tsx。一般 / PowerPoint / アシスタント / MCP の 4 タブ。旧記述: PowerPoint / アシスタントの 3 タブ） |
...
- **索引のスキーマ**は必要分だけ（links・tags・pinned・mtime_ns はある。
  `after:` / `before:` の検索は mtime_ns を見る。ULID id は持たない = パスが
  鍵、ADR-0042）。捨てられるキャッシュなので、機能追加時に世代を上げて拡張する
- **OboeGaki 側にしか無いもの**: フォルダの Drag & Drop 移動（ADR-0024 追記 7）、
  ...
  Word（.docx）書き出し（ADR-0059）、やること一覧（ADR-0056）、どこからでも
  書き取り（ADR-0057）、埋め込み（ADR-0058）、**MCP サーバ**（ADR-0051。
  `oboegaki-mcp` を `.app` に同梱し、Claude Desktop などから検索・参照・
  作成・追記・差し替え・移動・ゴミ箱ができる。`.mcp-ignore` で見せない
  フォルダを決められる。第 10 群で実装）、履歴の差分表示（ADR-0054）、
  一括置換（ADR-0055）、文字色（ADR-0061）、別名つきノートリンク（ADR-0064）、
  プレビューモード（ADR-0065）
