...
**本アプリの方針**: 先行アプリの _UX_（マーカーが隠れる、タグがインラインで書ける、サイドバーがタグツリー）は取り入れるが、_記法_ は **CommonMark + GFM 拡張**を基準とし、**保存形式も素の Markdown とする**（G3）。
...
7. **番号付きの字下げ**（ADR-0066、2026-09-17）: Tab / Shift+Tab で番号付きの項目の深さを変えたら、そのリスト（空行で切れるまで）の番号を振り直す。入れ子は 1 から、親は続きから。先頭の番号は保つ。Enter の継続（3.）は振り直さないまま。
...
| **全マーカー**                                           | —                                        | **プレビューモード（`Cmd+Shift+P`。[ADR-0065](adr/0065-wysiwyg-mode.md)）が ON のときは、書き込んでいる行だけ現す**（カーソルを置いただけ・選んだだけでは現さない） |
