...
- ✅ 対応済み（同日） 検索結果に世代ガードが無く、**遅いクエリの結果が後から新しい結果を上書き**
  （App.tsx:1198。`.catch` も無く索引故障時は無音）
- 🔶 一部対応（同日: openNote・履歴・ピン・一覧更新・検索に catch 追加。全数の網羅は App 層テスト導入と併せて） `async` ハンドラ 36 個中 try/catch は 17 個。`openNote`・新規作成・復元・
  完全削除・アシスタント等の失敗が**完全に無音**。`fetchLists` が 1 本失敗すると
  一覧更新イベント 3 種が全部無音で死ぬ
- ✅ 対応済み（同日） 仮身化（切り出し）の題名サニタイズが Rust とずれ（`/` `:` 等）、本文の
  `[[リンク]]` が実ファイル名と食い違い**クリックすると 2 つ目のノートができる**
  （extract.ts:21 — `createNote` の戻り値からリンクを組み直すべき）
- ✅ 対応済み（同日） 「保存した検索」は一覧ペイン（Cmd+2）を閉じていると**押しても無反応**
  （表示フラグが検索欄と別）
...
- ✅ 対応済み（同日） 環境設定の数値入力が無検証（空欄 → 0 / NaN がその場だけ効く）
- ✅ 対応済み（同日） ペイン幅ドラッグが `pointercancel` でリスナー残留
