...
- **[実証済み] 打鍵性能が基準割れ**（p95 = 17〜25ms、基準 16ms。2 回計測とも不合格）。
  原因は複合:
...
- **`autosave.flush()` が同期・保存は非同期、なのに全域が「flush してから次へ」前提**
...
- **競合ダイアログ表示中も自動保存の予約が生きている**（App.tsx:1644 の分岐だけ
...
- **`pendingSave` の完了処理がノートを取り違える**（App.tsx:1364）。ノート A の保存が
...
- **ゴミ箱移動時の mtime 打ち直しが黙って失敗し得る**（`vault.rs:864` — `if let Ok` +
  ...
  外す形が要る
- **履歴（唯一作り直せない資産）だけ非アトミック書き込み**（`history.rs:84` が素の
...
- **履歴の復元が直後に古い本文で上書きされ得る**（App.tsx:722 — `autosave.cancel()`
